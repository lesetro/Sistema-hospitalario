...
Factura.afterCreate(async (factura, options) => {
  const paciente = await sequelize.models.Paciente.findByPk(factura.paciente_id);
  if (paciente && paciente.usuario_id) {
    await sequelize.models.Notificacion.create({
      usuario_id: paciente.usuario_id,
      mensaje: `Se ha generado una nueva factura por $${factura.monto} con fecha ${factura.fecha_emision} 
      para el paciente ${paciente.nombre}.`,
      leida: false
    });
  }
});
